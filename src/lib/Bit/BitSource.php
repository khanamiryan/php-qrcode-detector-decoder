...
final class BitSource {
  ...
  /**
   * @param $bytes. from which this will read bits. Bits will be read from the first byte first.
  ...
  /**
   * @param $numBits int number of bits to read
   * @return int representing the bits read. The bits will appear as the least-significant
   *         bits of the int
   * @throws \Exception if numBits isn't in [1,32] or more than is available
   */
  public function readBits($numBits) {
    if ($numBits < 1 || $numBits > 32 || $numBits > $this->available()) {
      throw new \Exception(strval($numBits));
    }
