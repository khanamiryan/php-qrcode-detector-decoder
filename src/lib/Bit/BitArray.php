...
final class BitArray  {
    ...
    public function get($i) {
        $key = intval($i / 32);
        return intval($this->bits[$key] & (1 << ($i & 0x1F))) != 0;
    }
    ...
    public function set($i) {
        ...
        $this->bits[intval($i / 32)] = $this->common->overflow($this->bits[intval($i / 32)]);
    }
     ...
     * @return string. index of first bit that is set, starting from the given index, or size if none are set
     ...
     * @return string index of next unset bit, or {@code size} if none are unset until the end
     * @see #getNextSet(int)
     */
    public function getNextUnset($from) {
        ...
        while ($currentBits == 0) {
            ...
            $currentBits = $this->common->overflow(~$this->bits[$bitsOffset]);
        }
    ...
    public function setRange($start, $end) {
        ...
        for ($i = $firstInt; $i <= $lastInt; $i++) {
            ...
            $this->bits[$i] = $this->common->overflow($this->bits[$i]|$mask);
        }
    }
     ...
     * @return boolean true if all bits are set or not set in range, according to value argument
    ...
    public function isRange($start, $end, $value) {
        ...
        for ($i = $firstInt; $i <= $lastInt; $i++) {
            ...
            if ($firstBit == 0 && $lastBit == 31) {
                ...
                $mask = 0;
                for ($j = $firstBit; $j <= $lastBit; $j++) {
                    $mask = $this->common->overflow($mask|(1 << $j));
                }
            }
    ...
    /**
     * @return array underlying array of ints.
     * The first element holds the first 32 bits, and the least significant bit is bit 0.
     */
