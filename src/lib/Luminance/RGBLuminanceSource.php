...
// Calculate luminance cheaply, favoring green.
    ...
    function grayscale(){
        ...
        $ret = array_fill(0, $width*$height,0);
        for ($y = 0; $y < $height; $y++)
        {
            for ($x = 0; $x < $width; $x++)
            {
...
function getMiddleBrightnessPerArea($image)
{
    ...
    $minmax = array_fill(0,$numSqrtArea,0);
    for ($i = 0; $i < $numSqrtArea; $i++)
    {
        $minmax[$i] = array_fill(0,$numSqrtArea,0);
...
function grayScaleToBitmap ($grayScale)
{
    ...
    $bitmap = array_fill(0,$this->dataWidth*$this->dataHeight,0);
