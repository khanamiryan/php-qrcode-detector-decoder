...
final class Result {
    ...
    /**
     * @return string raw bytes encoded by the barcode, if applicable, otherwise {@code null}
     */
    ...
    /**
     * @return string points related to the barcode in the image. These are typically points
    ...
    public function addResultPoints($newPoints) {
        ...
        elseif($newPoints != null && count($newPoints) > 0) {
            $allPoints = $this->common->array_fill(0,count($oldPoints)+count($newPoints),0);
            $allPoints = $this->common->arraycopy($oldPoints, 0, $allPoints, 0, count($oldPoints));
            $allPoints = $this->common->arraycopy($newPoints, 0, $allPoints, count($oldPoints), count($newPoints));
