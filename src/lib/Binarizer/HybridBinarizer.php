...
final class HybridBinarizer extends GlobalHistogramBinarizer{
     ...
     * @param $matrix
     */
    private static function calculateThresholdForBlock($luminances, $subWidth, $subHeight, $width, $height, $blackPoints, $matrix) {
        for ($y = 0; $y < $subHeight; $y++) {
            $yoffset = intval($y << self::$BLOCK_SIZE_POWER);
            $maxYOffset = $height - self::$BLOCK_SIZE;
            if ($yoffset > $maxYOffset) {
                $yoffset = $maxYOffset;
            }
            for ($x = 0; $x < $subWidth; $x++) {
                $xoffset = intval($x << self::$BLOCK_SIZE_POWER);
            ...
            for ($x = 0; $x < $subWidth; $x++) {
                ...
                for ($yy = 0, $offset = $yoffset * $width + $xoffset; $yy < self::$BLOCK_SIZE; $yy++, $offset += $width) {
                    for ($xx = 0; $xx < self::$BLOCK_SIZE; $xx++) {
                        $pixel = intval(intval($luminances[intval($offset +$xx)]) & 0xFF);
                        $sum += $pixel;
                    ...
                    if ($max - $min > self::$MIN_DYNAMIC_RANGE) {
                        ...
                        for ($yy++, $offset += $width; $yy < self::$BLOCK_SIZE; $yy++, $offset += $width) {
                            for ($xx = 0; $xx < self::$BLOCK_SIZE; $xx++) {
                                $sum += intval($luminances[$offset +$xx] & 0xFF);
                            }
                        }
                    }
                }
                ...
                $average = intval($sum >> (self::$BLOCK_SIZE_POWER * 2));
                if ($max - $min <= self::$MIN_DYNAMIC_RANGE) {
